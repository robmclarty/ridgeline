...
- Remove the manual process.on('SIGINT', ...) in src/cli.ts once fascicle's runner default install_signal_handlers handles abort + cleanup; migrate teardown to ctx.on_cleanup registrations
...
**Relevant modules:**
- src/cli.ts — entry; remove manual SIGINT handler once fascicle install_signal_handlers covers every command
