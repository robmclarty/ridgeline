...
**Decision.** Ridgeline detects greywall at startup and uses it as the sandbox provider when available. Sandbox configuration propagates to all Claude invocations, including both specialists and the synthesizer. Network access is denied by default and granted only via explicit allowlists.
...
**Why this approach.** LLM-generated code executes in the user's environment with the user's permissions. Without sandboxing, a single hallucinated `rm -rf` or exfiltration attempt can cause irreversible damage. Mandatory sandboxing would break adoption on platforms where greywall is not installed. User-configured sandboxing adds friction and is easily forgotten. Auto-detection with graceful degradation follows the principle of least privilege[^14]: apply the available constraint by default, warn when it is unavailable, never silently escalate privileges. The provider abstraction (`SandboxProvider` interface with `checkReady`) keeps the door open for additional sandbox backends without changing invocation code. OWASP sandboxing guidance[^15] recommends defense-in-depth with network isolation as the primary control, which aligns with Ridgeline's default-deny network policy and explicit `networkAllowlist` for phases that genuinely need external access (e.g., research phases). Greywall is the chosen provider because it supports domain-level allowlisting (rather than binary network on/off) and works cross-platform on macOS and Linux.
