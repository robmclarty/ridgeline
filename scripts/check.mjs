 ...
 *   node scripts/check.mjs --json             Machine-readable summary on stdout
 *   node scripts/check.mjs --bail             Stop at the first failure
 *   node scripts/check.mjs --only types,lint  Run only the named checks
 *   node scripts/check.mjs --skip docs,spell  Skip the named checks
 ...
 *
 * Each tool writes its raw output to .check/<name>.{json,stdout.txt,stderr.txt}.
 * The orchestrator never parses diagnostics itself; agents read the tool output
 * directly. This keeps the orchestrator dumb and upgrade-proof.
 */
...
const CHECKS = [
  {
    ...
    command: "npx",
    args: ["tsc", "--noEmit", "--pretty", "-p", "tsconfig.check.json"],
    output_file: null,
  },
  {
    name: "lint",
  ...
  },
  {
    ...
    command: "npx",
    args: ["ast-grep", "scan", "--json=compact"],
    output_file: "struct.json",
  },
  {
    name: "agents",
    description: "Agent/skill metadata (agnix)",
    command: "npx",
    args: ["agnix"],
    output_file: null,
  },
  {
    name: "dead",
    description: "Fallow: dead code, cycles, duplication, complexity",
    command: "npx",
    args: ["fallow"],
    output_file: null,
  },
  ...
  {
    name: "spell",
    description: "Spell check (cspell)",
    command: "npx",
    args: ["cspell", "--no-progress", "--no-summary", "--reporter=default"],
    output_file: null,
  },
  {
    name: "test",
 ...
 * (validated), everything else writes raw text to .check/<name>.{stdout,stderr}.txt.
 */
...
function select_checks(all, only, skip, include, run_all) {
  const skip_set = new Set(skip ?? [])
  const include_set = new Set(include ?? [])
  return all.filter((c) => {
    if (only) return only.includes(c.name)
    if (skip_set.has(c.name)) return false
    if (c.opt_in && !run_all && !include_set.has(c.name)) return false
    return true
  })
}
