...
program
  ...
  .option("--verbose", "Stream builder/evaluator output to terminal", false)
  .option("--timeout <minutes>", "Max duration per phase in minutes", "30")
  .option("--max-retries <n>", "Max evaluator retry loops per phase", "2")
  .option("--check <command>", "Baseline check command (overrides constraints.md)")
  .option("--model <name>", "Model for builder and evaluator", "opus")
