...
describe("invokeEnsemble — default 2-specialist", () => {
  ...
  it("continues with a lone survivor when one specialist times out", async () => {
    ...
    const specialists = [makeSpecialist("a"), makeSpecialist("b")]
    const result = await invokeEnsemble(buildInvoke(specialists))
  ...
  it("halts when all specialists fail", async () => {
    ...
    const specialists = [makeSpecialist("a"), makeSpecialist("b")]
    await expect(invokeEnsemble(buildInvoke(specialists))).rejects.toThrow(/all 2 specialists fail/)
  })
  ...
  it("logs specialist timeouts to trajectory.jsonl with reason: timeout", async () => {
    ...
    const specialists = [makeSpecialist("a"), makeSpecialist("b")]
    await invokeEnsemble(buildInvoke(specialists, { buildDir: tmp }))
  ...
  it("honors a custom specialistTimeoutSeconds", async () => {
    ...
    const specialists = [makeSpecialist("a"), makeSpecialist("b")]
    await invokeEnsemble(buildInvoke(specialists, { specialistTimeoutSeconds: 420 }))
...
describe("invokeEnsemble — --thorough cross-annotation pass", () => {
  beforeEach(() => { mockClaude.mockReset() })
  ...
  it("invokes 3 specialists + 3 annotations + 1 synthesizer = 7 calls", async () => {
    mockClaude.mockImplementation(async () => makeClaudeResult("body"))
    await invokeEnsemble(makeConfig())
...
describe("invokeEnsemble — agreement-based synthesis skip", () => {
  beforeEach(() => { mockClaude.mockReset() })
  ...
  it("skips synthesizer when skeletons agree", async () => {
    ...
    await invokeEnsemble(buildConfig({ onAgreementSkip }))
  ...
  it("logs a synthesis_skipped trajectory entry when synthesis is skipped", async () => {
    ...
    await invokeEnsemble(buildConfig({ onAgreementSkip, buildDir: tmp }))
